...
| `expression-2` | `<code>.avatar` | `bithuman run <file>` on macOS (Apple Silicon); on Linux x86_64 after `bithuman engine install linux`; the browser via [`?render=local`](/guides/browser-rendering); bitHuman cloud. Also the [`Expression2` Swift product](/sdk/swift#expression-2-on-device) on a Mac, iPhone or iPad — measured 2026-09-09, this same file rendered 416x720 frames on an iPhone 15 — but not on its own: the artifact does not carry `w2v_frontend_cpuAndNE.mlpackage`, so you pass the directory `bithuman engine install mac` writes as `sharedEngineDir:`. The whole app is [Swift / iOS — a talking avatar on the iPhone you have](/examples/swift-ios-expression2). (This row used to say the Swift engine "wants a per-identity CoreML bundle, which is a different artifact and is not published". That was true before tap `2.6.0` added the container reader; it is not true now.) |
