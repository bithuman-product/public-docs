...
| `essence-2-max` | `<code>.pkl` | Recognised, and handed to the [cloud](/concepts/essence-2-max) — GPU-only by design |
...
`--model` (`essence-1`, `essence-2`, `essence-2-max`, `expression-2`), or read
...
Essence 2 bundle reports `essence2-light`, an Essence 2 Max bundle
`essence2-quality` — and is never a valid `model` value; see
...
`pip install bithuman-cli` also exists, but only on macOS Apple Silicon (there
are no Linux or Intel-Mac wheels). The separate `bithuman` PyPI package is the
[Python library](/sdk/python), not the CLI.
