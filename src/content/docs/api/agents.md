...
| `expression-2` | `.avatar` | The per-identity Expression 2 artifact. Sizes range widely, so read `Content-Length` rather than budgeting from a figure on this page. **The `.avatar` extension is historical: it is the frozen back-compat alias of `.imx`, not a distinct encoding.** A few of the oldest agents still carry an older container form, so check with `bithuman open <file>` rather than assuming. [Runs locally](/sdk/cli/reference) on macOS (Apple Silicon) and Linux x86_64; also in the browser via [`?render=local`](/guides/browser-rendering), and served on bitHuman's cloud. |
