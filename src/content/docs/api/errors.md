...
| `503` | Service Unavailable | All workers busy — retry with backoff. Also `MODEL_NOT_YET_AVAILABLE` — a second-generation family temporarily paused for your account (rare — Essence 2 / Expression 2 are GA since July 10, 2026). Transient in either case; all five avatar models render [talking video](/api/video) today. |
...
| `MODEL_NOT_YET_AVAILABLE` | 503 | Essence 2 / Expression 2 are **GA** (since July 10, 2026), so [creation](/api/agents#generate-an-agent) and [model add](/api/agents#add-a-model-to-an-existing-agent) don't return this in normal operation — it's the safety response if a v2 family is temporarily paused. All five avatar models render [talking video](/api/video) today, so a `503` there is transient too. Nothing charged; retry later or use another model. |
