...
|---|---|---|
| Python SDK (`bithuman`) | **3.1.5** — the 2.x line ends at 2.9.0 on PyPI, so a `bithuman<3` pin is a downgrade, not a hold | [PyPI](https://pypi.org/project/bithuman/) |
...
| Swift SDK (`Essence2`) | ships with the package — pin the package version on [Install](/sdk/ios#install) and the engine comes with it. Essence 2 in your own iOS or macOS app works from **2.13.2** — it opens the `<code>.imx` you download here. | [SwiftPM](https://github.com/bithuman-product/homebrew-bithuman) |
...
| Android AAR (`ai.bithuman:essence2-android`) | **0.5.5** (`0.2.0` through `0.5.3` stay on Central and are superseded — `0.5.1` and `0.5.2` cannot install a model on a handset; pin `0.5.5`) | [Maven Central](https://repo1.maven.org/maven2/ai/bithuman/essence2-android/) |
