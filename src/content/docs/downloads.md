...
|---|---|---|
| [CLI](/sdk/cli) — macOS Apple Silicon, Linux x86_64 | `curl -fsSL https://raw.githubusercontent.com/bithuman-product/homebrew-bithuman/main/install.sh \| sh` | `bithuman login`, then `bithuman run` — `render` needs a credential, and so does `run` on macOS |
...
|---|---|---|
| Python SDK (`bithuman`) | **3.1.8** — the 2.x line ends at 2.9.0 on PyPI, so a `bithuman<3` pin is a downgrade, not a hold | [PyPI](https://pypi.org/project/bithuman/) |
...
| Android AAR (`ai.bithuman:essence2-android`) | **0.5.6** (`0.2.0` through `0.5.5` stay on Central and are superseded — `0.5.1` and `0.5.2` cannot install a model on a handset; pin `0.5.6`) | [Maven Central](https://repo1.maven.org/maven2/ai/bithuman/essence2-android/) |
