...
| `?model=` tier pin appears ignored | Unrecognized tier slugs **fall back silently** to the agent's default routing. | Check the slug against the model's guide ([Expression 2](/concepts/expression-2#serving-tiers), [Essence 2](/concepts/essence-2#serving-tiers) — force tiers are `essence-2-gpu/-apple/-cpu`, `expression-2-gpu/-cpu/-apple`; the older `-ane` spelling stays accepted); for production, omit `?model=`. |
