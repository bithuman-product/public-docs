...
| Symptom | Cause | Fix |
|---|---|---|
| `400 VALIDATION_ERROR` — `Invalid model '<x>'; must be one of: …` | Unknown or retired `model` value. The error lists the accepted names. | Use one of `essence-1`, `essence-2-quality`, `essence-2-light`, `essence-2`, `expression-1`, `expression-2`, `auto` (or the bare `essence` / `expression` shorthands). No credits are charged. |
