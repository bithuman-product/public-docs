...
Grab a free dev key at [bithuman.ai → Developer](https://www.bithuman.ai/#developer); it lands in your inbox in seconds with the 99-credit free tier attached.
...
| **`BITHUMAN_UNMETERED=1` dev mode** | either | **Free** | Skips the auth + heartbeat — dev / parity testing only |
...
A "credit minute" is wall-clock time the engine is actively producing frames (or, on-device, the wall-clock between `chat.start()` and `chat.stop()` with avatar attached). Idle / paused / disconnected time isn't billed.

## Self-hosted vs metered
...
| **Unmetered dev mode** | `BITHUMAN_UNMETERED=1` skips the auth + heartbeat entirely. For local dev, CI, contract tests, and parity work — not licensed for production. | none |
| **Audio-only** | Swift SDK with no `AvatarConfig` attached. Fully offline, never reaches the auth endpoint. | none |

The CLI, Python SDK, and Docker container all honour
`BITHUMAN_UNMETERED=1`. The Swift SDK has the same escape hatch via
the unmetered initializer documented in the [Swift SDK
reference](/sdks/swift).
...
- [Device matrix](/getting-started/device-matrix) — which platforms ship in 2.3 and which are pending.
- [Compatibility](/getting-started/compatibility) — SDK ↔ libessence ABI pinning across releases.
- [releases.bithuman.ai](https://releases.bithuman.ai) — pre-built wheels and install commands for every platform.
