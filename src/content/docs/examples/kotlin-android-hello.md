...
This page is the minimum runnable Android program that drives the
bitHuman avatar runtime on-device. Real-time lip-sync compositing on
`arm64-v8a` phones and tablets, no cloud inference. Compatible with
Android 10+ (API 29+).
...
- A bitHuman API secret exported via env var or read by your app at
  startup. Sign in at
  [www.bithuman.ai → Developer → API Keys](https://www.bithuman.ai/#developer).
- A `.imx` model file pushed to the device. The sample app uses
  `getExternalFilesDir(null)`; adapt to your storage scheme as needed.
...
That's the whole loop: 16 kHz mono PCM in, lip-synced `Bitmap`s out at
25 fps, fully on-device.
