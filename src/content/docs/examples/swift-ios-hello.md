...
>
> The package also vends **`Expression2`** — a separate product and a separate engine ([`expression-2`](/concepts/expression-2)) — which *has* rendered on an iPhone. As of **2.6.0** it takes a model path and can open the `.avatar` you download for your own agent; it still ships no weights of its own. This example does not use it: see [Expression 2 on-device](/sdk/ios#minimal-code). Since **2.7.0** it also vends **`Essence2`**, the [`essence-2`](/concepts/essence-2) engine's C interface, which builds for iOS device, iOS simulator and macOS; since **2.13.2** it opens the `.imx` you download for your own agent, on iPhone and Mac alike, though you still fetch the file yourself: [Essence 2 on-device](/sdk/ios#install).
