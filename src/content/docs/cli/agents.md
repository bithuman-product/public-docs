...
```json
{"cli":"2.3.21","libessence":"2.3.6","abi":7}
```
...
```json
{"ready":false,
 "versions":{"libessence":"2.3.6","abi":7,"cli":"2.3.21","wheel":null,"agent_python":null},
...
at least one of `prompt`/`image`/`video`/`audio`) so it can't silently spend.
