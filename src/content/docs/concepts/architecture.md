...
| Swift SDK (`Essence2`) | engine release `essence2-v1.4.0`, package 2.10.0 | SwiftPM | — (C interface; ONNX Runtime 1.26.0) |
| Rust SDK (`bithuman`) | in-tree crate, versioned with the CLI | source-only (not on crates.io) | v7 |
| bitHuman CLI | 2.6.4 (Homebrew / universal installer — macOS arm64 and Linux x86_64, both with the Essence 2 runtime inside); 2.3.25 (PyPI, macOS only) | Homebrew · PyPI · universal installer | v7 |
...
| Self-hosted CPU (your servers) | [Offline rendering, SDK 2.9.0+](/guides/deploy-self-hosted#essence-2-self-hosted--cpu-offline-rendering-sdk-290) (~22–31 FPS on 16 cores); [CLI local rendering](/sdk/cli/overview#essence-2-on-your-own-machine) — `render` and `run` — on macOS arm64 and Linux x86_64 (2.6.1); live streaming via cloud | — | [CLI local rendering](/sdk/cli/overview#local-rendering-by-platform) (macOS arm64, Linux x86_64) |
| On-device Apple Silicon (Mac / iOS) | The [CLI](/sdk/cli/overview#essence-2-on-your-own-machine) renders a downloaded `<code>.imx` on macOS Apple Silicon (2.6.1); in your own app, the [Swift SDK](/sdk/swift#essence-2-on-device) `Essence2` product (package 2.8.0) — a C interface, builds for iOS and macOS; no in-app model download route yet | — (cloud-only) | [Swift SDK](/sdk/swift) `Expression2`, v2.5.0+ — `macos-arm64` **and** `ios-arm64`, both proven on hardware, but engine only, [no model bundle published](/sdk/swift#expression-2-on-device) |
