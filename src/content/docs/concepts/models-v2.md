...
> tier runs on *bitHuman's* Apple Silicon through CoreML, reached over the
> network like any other cloud tier. The two places Essence 2 runs off our cloud are your own
...
| bitHuman cloud — Apple Silicon (CoreML) | ✅ chain tier — runs on the Mac **GPU** ([why](/concepts/essence-2#which-apple-compute-unit-runs-essence-2)) | — | ✅ chain tier — **split**: audio front end on the Neural Engine, per-frame rendering on the GPU ([why](/concepts/expression-2#which-apple-compute-units-run-expression-2)) |
