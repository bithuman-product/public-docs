...
description: "bitHuman's second-generation avatar models — essence-2 and expression-2 — real-time avatar video with on-device, browser, and cloud serving, per-minute pricing, and which to choose."
section: concepts
group: "Models"
order: 1
...
**`expression-2`**, are created through the API
...
integrations. The [gallery](https://bithuman.ai/explore?gallery=v2) shows the
...
- **[`essence-2`](/concepts/essence-2)** — the **standard** Essence model for
  ...
  **in the viewer's browser**, where frames never leave that
  ...
  Silicon and Linux x86_64), the current [`bithuman` wheel](/sdk/python), and
  the [Android AAR](/sdk/android).
...
(the current `bithuman` wheel), and **in the viewer's browser** where you opt a session
...
> machine** through the [CLI](/sdk/cli#what-renders-locally-and-where)
...
identity image itself is generated from your prompt; an uploaded `image` is
treated as a **reference** and always regenerated to standardize it — never
used raw. The identity video is generated internally from that image. Video
...
- **`aspect_ratio`** — `16:9` (default), `9:16`, or `1:1`. Drives **both** the
  generated identity image and the identity video; images are generated at 1080p.
...
(`/speak`, `/add-context`), and the [SDKs](/sdk).
...
| Self-hosted (your servers, CPU) | ✅ frames and whole clips through the current `bithuman` wheel, metered ([quickstart](/guides/deploy-self-hosted#essence-2-on-your-own-cpu)); ✅ local rendering via the [CLI](/sdk/cli#what-renders-locally-and-where) — `render` and `run` — on macOS Apple Silicon and Linux x86_64 (2.6.1). There is [no self-hosted live-worker image](/guides/self-host-local) for this generation | Local rendering via the [CLI](/sdk/cli#what-renders-locally-and-where) (macOS Apple Silicon, Linux x86_64); frames through the `bithuman[expression-2]` wheel; on Android through the [AAR](/sdk/android) |
| On-device macOS / iOS (Apple Silicon) | The [CLI](/sdk/cli#what-renders-locally-and-where) renders a downloaded `.imx` on macOS Apple Silicon (2.6.1); in your own app, the [Swift SDK](/sdk/ios#install) `Essence2` product renders the downloaded `.imx` on both iPhone and Mac from **2.13.2**. You fetch the file yourself; there is no in-app download route. | [Swift SDK](/sdk/ios) `Expression2` product — a Mac slice **and** an iPhone slice, both proven on real hardware; engine only, and it [opens the `.imx` you download](/sdk/ios#minimal-code) from the [download endpoint](/api/agents#download-an-agents-model) |
...
A few things to know today: essence-2's **in-browser** tier renders on the
viewer's own device (audio and frames never leave the browser) and is rolling
out per identity. **Local playback of a downloaded artifact** is now
...
(the [Docker image](/guides/deploy-self-hosted) is Expression 1 only) — a
hosted live session runs through the cloud or the browser, and on one machine
the CLI's [`bithuman run`](/sdk/cli) serves a live session locally. Measured
frame rates per platform are on the [performance page](/sdk/performance).
