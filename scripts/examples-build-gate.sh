...
# the gate cannot live in this repo's CI. This runner is scheduled on a Linux host
# (where the pager is) and executes the build over ssh on a Mac build host that has
# Xcode, a JDK 17 and an Android SDK with platform 35.
#
#   scheduling host (cron, the pager)  --ssh-->  $GATE_REMOTE_HOST (toolchains)
#
# CONFIGURATION. This repository is public, so the runner names no host: the
# build host comes from GATE_REMOTE_HOST (required for a build; without it the
# run is UNPROVEN, exit 2), and the paths default under $HOME. The scheduler's
# private configuration sets them.
#
...
#   2  UNPROVEN: the extractor's own self-test failed, the build host was unreachable,
...
# `set -x`. The pager reads its own credentials in its own process; nothing
# sensitive crosses the ssh hop.

set -u -o pipefail

CHECKOUT="${GATE_CHECKOUT:-$HOME/docs-gate/public-docs}"
REMOTE="${GATE_REMOTE_HOST:-}"
# The token the remote prints its own exit status behind. See the sentinel note
# at the run step: on the build host, ssh's exit status is not a signal.
RC_SENTINEL="EXAMPLES_BUILD_GATE_RC"
WORKDIR_LOCAL="${TMPDIR:-/tmp}"
RECEIPT="${GATE_RECEIPT:-$HOME/scripts/docs-examples-build.receipt.json}"
...
# >>> measurement lock snippet (the build host's lock files and rules; --selftest evals THIS copy)
# ★2026-09-14: this gate is not the only scheduled job that reaches the build host, and that
# night a sibling disk walker ran find/stat there inside a graded measurement window and
# VOIDED its control run.  A measurement on the remote
...
# `command -v node`. cron on the scheduling host hands a job PATH=/usr/bin:/bin, and this
...
#
#   $ env -i HOME=$HOME PATH=/usr/bin:/bin sh -c '.../examples-build-gate.sh --selftest'
...
# absolute homes node actually takes on Linux and macOS.
resolve_node() {
...
#
# check-published-examples-build.mjs is named by NO schedule line — the host-tool registry
...
#   * a hand-edit to the .mjs in the scheduled checkout, and
...
# origin/main, the same standard every other scheduled host tool is held to.
...
# ---------------------------------------------------------- build host checks ---
if [ -z "$REMOTE" ]; then
  say "UNPROVEN: GATE_REMOTE_HOST is not set — no build host to compile on, so nothing was compiled"
  exit 2
fi
...
# $REMOTE. Before 2026-09-11 this could not have passed: `ssh "$REMOTE" "exit 7"`
...
# end of an scp. scp exits 0 on a copy it truncated (a full /tmp on the build host, a
...
# Some macOS ssh servers exec the command under `/usr/bin/login -f -pq ...`, and
# login(1) does not propagate its child's status. MEASURED on the build host:
# `ssh "$REMOTE" "exit 3"` -> 0, `ssh "$REMOTE" false` -> 0.
