...
// WHY
// ---
// MEASURED 2026-09-11, multiplexing off: on some macOS hosts
//
//     ssh <mac> "exit 3"   -> 0
//     ssh <mac> false      -> 0
//
// while plain OpenSSH hosts answer 3 and 1. Those Macs' ssh server execs the
// command under `/usr/bin/login -f -pq -h <ip> <user> /bin/zsh -c '<cmd>'`, and
// login(1) does not propagate its child's exit status. Proved on one such host,
// one command apart:
//
//     sudo /usr/bin/login -f -pq -h 1.2.3.4 $u /bin/zsh -c 'exit 3'  -> 0
//                                           /bin/zsh -c 'exit 3'     -> 3
//
// This repo's compiling gate runs on such a Mac. Until 2026-09-11 its verdict was
...
if (findings.length) {
  ...
  console.error(`
On some macOS hosts that status is ALWAYS 0 (their ssh server runs the command
